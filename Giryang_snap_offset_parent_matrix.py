...
def snap_offset_parent_matrix():
    ...
    joint_world_mtx = cmds.xform("joint1", query=True, worldSpace=True, matrix=True)
    joint_math_mtx = om.MMatrix(joint_world_mtx)
    # nurbsCircle1 can be targetObj or offset group for snapping
    target_world_mtx = cmds.xform("nurbsCircle1", query=True, worldSpace=True, matrix=True)
    ...
    cmds.setAttr('nurbsCircle1.offsetParentMatrix',joint_math_mtx, type='matrix')
